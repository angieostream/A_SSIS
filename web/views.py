...
def students():
    if request.method == 'POST':
        ...
        if create_student(id, firstname, lastname, course, year, gender):
            flash('Student added successfully!', 'success')
            return redirect(url_for('views.students'))
        else:
            flash('ID number already exists. Please use a different ID.', 'danger')
            return render_template('students.html', student=get_students(), programs=get_programs(), stay_open=True)

    students_list = get_students()
    programs = get_programs()  # Fetch the list of programs for the dropdown
